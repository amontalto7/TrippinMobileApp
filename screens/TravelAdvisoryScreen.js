...
import { Button, View, List, ListItem, WebView } from "react-native";
const exampleCountry = {
  ...
  "level": 4,
  "link": "http://travel.state.gov/content/travel/en/traveladvisories/traveladvisories/venezuela-travel-advisory.html",
...
export default class TravelAdvisoryScreen extends Component {
  ...
  pullCountriesList() {
    this.setState;
    Axios.get(travelAdvisoryApiUrl)
      .then(response => {
    ...
    return (
      <View>
        {countriesArray.map((item, i) => (
          <ListItem key={i} title={item.country} />
        ))}
      </View>
    );
  }
  ...
  render() {
    return <WebView source={{ html: exampleCountry.content }} />;
  }
}
